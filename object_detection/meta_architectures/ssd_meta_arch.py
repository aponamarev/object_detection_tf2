...
from object_detection.core.box_predictor import KerasBoxPredictor
...
class SSDKerasFeatureExtractor(tf.keras.Model):
  ...
  def restore_from_tf_checkpoint_fn(self, reader) -> dict:
    raise NotImplementedError()
...
class SSDMetaArch(model.DetectionModel):
  ...
  def predict(self, preprocessed_inputs, true_image_shapes):
    ...
    if self._feature_extractor.is_keras_model:
      ...
      raise NotImplementedError("Feature maps extraction has to be implemented in Keras")
    ...
    if self._box_predictor.is_keras_model:
      ...
      raise NotImplementedError("Box predictor has to be implemented in Keras")
    predictions_dict = {
...
class KerasPreprocessingLayer(tf.keras.layers.Layer):

  _image_resizer_fn: Callable = None

  def __init__(self, image_resizer_fn: Callable, name: str = 'Preprocessor', **kwargs):

    super(KerasPreprocessingLayer, self).__init__(name=name, **kwargs)

    ######################################
    ####          Exceptions          ####
    ######################################

    if not isinstance(image_resizer_fn, Callable):
      raise ValueError("image_resizer_fn should be a callable function")
...
class KerasSSDPostprocessingLayer(tf.keras.layers.Layer):

  _box_coder: BoxCoder = None
  _score_conversion_fn: Callable = None
  _non_max_suppression_fn: Callable = None
  _add_background_class: bool = None


  def __init__(self,
               box_coder: BoxCoder,
               score_conversion_fn: Callable,
               non_max_suppression_fn: Callable,
               add_background_class: bool = True,
               explicit_background_class: bool = False,
               name: str = "Postprocessor",
               **kwargs):
    """
    Layer that converts prediction tensors to final detections.

    This layer converts raw predictions tensors to final detection results by
    slicing off the background class, decoding box predictions and applying
    non max suppression and clipping to the image window.
    ...
    :param anchors: a list of default bounding boxes.
    :param box_coder: a box_coder.BoxCoder object.
    :param score_conversion_fn: callable elementwise nonlinearity (that takes tensors
        as inputs and returns tensors).  This is usually used to convert logits
        to probabilities.
    :param non_max_suppression_fn: batch_multiclass_non_max_suppression
        callable that takes `boxes`, `scores` and optional `clip_window`
        inputs (with all other inputs already set) and returns a dictionary
        hold tensors with keys: `detection_boxes`, `detection_scores`,
        `detection_classes` and `num_detections`. See `post_processing.
        batch_multiclass_non_max_suppression` for the type and shape of these
        tensors.
    :param add_background_class: Whether to add an implicit background class to
        one-hot encodings of groundtruth labels. Set to false if training a
        single class model or using groundtruth labels with an explicit
        background class.
    :param explicit_background_class: Set to true if using groundtruth labels with an
        explicit background class, as in multiclass scores.
    :param name: a layer name
    :param kwargs:
    """

    super(KerasSSDPostprocessingLayer, self).__init__(name=name, **kwargs)

    ######################################
    ####          Exceptions          ####
    ######################################

    if not isinstance(box_coder, BoxCoder):
      raise ValueError("box_coder should be of BoxCoder type")

    if not isinstance(score_conversion_fn, Callable):
      raise ValueError("score_conversion_fn should be a callable function")

    if not isinstance(non_max_suppression_fn, Callable):
      raise ValueError("non_max_suppression_fn should be a callable function")

    if not isinstance(add_background_class, bool):
      raise ValueError("add_background_class should be of bool type")

    if not isinstance(explicit_background_class, bool):
      raise ValueError("explicit_background_class should be of bool type")
  ...
  def call(self, prediction_dict, true_image_shapes):
    """Converts prediction tensors to final detections.
    ...
    with tf.name_scope(self.name):
      ...
      if 'anchors' in prediction_dict:
        anchors = prediction_dict['anchors']
      anchor_indices = tf.range(anchors.shape[0])
      batch_anchor_indices = tf.tile(
          tf.expand_dims(anchor_indices, 0), [batch_size, 1])
      # All additional fields need to be float.
      additional_fields.update({
  ...
  def _compute_clip_window(self, preprocessed_images, true_image_shapes):
    ...
    """
    with tf.name_scope("clip_window"):
      if true_image_shapes is None:
        return tf.constant([0, 0, 1, 1], dtype=tf.float32)
...
class KerasSSDLossLayer(tf.keras.layers.Layer):

  _expected_loss_weights_fn: Callable = None
  _classification_loss: losses.Loss = None
  _localization_loss: losses.Loss = None
  _classification_loss_weight: float = None
  _localization_loss_weight: float = None

  def __init__(self,
               classification_loss: losses.Loss,
               classification_loss_weight: float,
               localization_loss: losses.Loss,
               localization_loss_weight: float,
               expected_loss_weights_fn: Callable = None,
               name: str = "SSDLoss",
               **kwargs):

    super(KerasSSDLossLayer, self).__init__(name=name, **kwargs)

    ######################################
    ####          Exceptions          ####
    ######################################
    if not isinstance(classification_loss, losses.Loss):
      raise ValueError("classification_loss should be an object_detection.core.losses.Loss object")

    if not isinstance(classification_loss_weight, float):
      raise ValueError("classification_loss_weight should be a float object")

    if not isinstance(localization_loss, losses.Loss):
      raise ValueError("localization_loss should be an object_detection.core.losses.Loss object")

    if not isinstance(localization_loss_weight, float):
      raise ValueError("localization_loss_weight should be a float object")

    if (expected_loss_weights_fn is not None) and (not isinstance(expected_loss_weights_fn, Callable)):
      raise ValueError("expected_loss_weights_fn should be a Callable object")
  ...
  def call(self, prediction_dict: dict, ground_truth_dict: dict):
    """Compute scalar loss tensors with respect to provided groundtruth.
    ...
    # with tf.name_scope(scope, 'Loss', prediction_dict.values()):
    keypoints = None
    if fields.BoxListFields.keypoints in ground_truth_dict:
      keypoints = ground_truth_dict[fields.BoxListFields.keypoints]
    weights = None
    if fields.BoxListFields.weights in ground_truth_dict:
      weights = ground_truth_dict[fields.BoxListFields.weights]
    confidences = None
    if fields.BoxListFields.confidences in ground_truth_dict:
      confidences = ground_truth_dict[fields.BoxListFields.confidences]
    (batch_cls_targets, batch_cls_weights, batch_reg_targets,
     batch_reg_weights, batch_match) = self._assign_targets(
         self.groundtruth_lists(fields.BoxListFields.boxes),
         self.groundtruth_lists(fields.BoxListFields.classes),
    ...
    losses_mask = None
    if fields.InputDataFields.is_annotated in ground_truth_dict:
      losses_mask = tf.stack(ground_truth_dict[
          fields.InputDataFields.is_annotated])
    location_losses = self._localization_loss(
        prediction_dict['box_encodings'],
        batch_reg_targets,
...
class KerasSSDMetaArch(tf.keras.Model):

  width: int = None
  height: int = None
  feature_extractor: SSDKerasFeatureExtractor = None
  preprocessor: KerasPreprocessingLayer = None
  postprocessor: KerasSSDPostprocessingLayer = None
  box_predictor: KerasBoxPredictor = None
  _anchor_generator: AnchorGenerator = None
  _anchors: box_list.BoxList = None

  def __init__(self,
               width: int,
               height: int,
               feature_extractor: SSDKerasFeatureExtractor,
               box_predictor: KerasBoxPredictor,
               anchor_generator: AnchorGenerator,
               image_resizer_fn: Callable,
               box_coder: BoxCoder,
               score_conversion_fn: Callable,
               non_max_suppression_fn: Callable,
               add_background_class: bool = True,
    ...
    super(KerasSSDMetaArch, self).__init__(name=name, **kwargs)
    ...
    if not isinstance(anchor_generator, AnchorGenerator):
      raise ValueError("anchor_generator should be of AnchorGenerator type")

    if not isinstance(box_predictor, KerasBoxPredictor):
      raise ValueError("box_predictor should be of KerasBoxPredictor type")
    ...
    self.preprocessor = KerasPreprocessingLayer(
            image_resizer_fn=image_resizer_fn,
            **kwargs
    )
    self.postprocessor = KerasSSDPostprocessingLayer(
            box_coder=box_coder,
            score_conversion_fn=score_conversion_fn,
            non_max_suppression_fn=non_max_suppression_fn,
            add_background_class=add_background_class,
            explicit_background_class=explicit_background_class,
            **kwargs
    )
  ...
  def build(self, input_shape):
    ...
    input_shape = tf.TensorShape((None, self.height, self.width, 3))
    self.feature_extractor.build(input_shape)
    feature_map_spatial_dims = self.feature_extractor.compute_output_shape(
            input_shape
    )
    shape_tuples = [tuple(s)[1:3] for s in feature_map_spatial_dims]

    self._anchors = box_list_ops.concatenate(
            self._anchor_generator.generate(
                    shape_tuples,
                    im_height=input_shape[1],
                    im_width=input_shape[2]
            )
    )
    super(KerasSSDMetaArch, self).build(input_shape)
  ...
  def call(self, inputs, training=False):
    """Predicts unpostprocessed tensors from input tensor.
      ...
      Args:
        inputs: a [batch, height, width, channels] image tensor.
        true_image_shapes: int32 tensor of shape [batch, 3] where each row is
          of the form [height, width, channels] indicating the shapes
          of true images in the resized images, as resized images can be padded
          with zeros.
    ...
    scaled_inputs = self.feature_extractor.preprocess(inputs)
    resized_inputs, true_image_shapes = self.preprocessor(scaled_inputs)

    feature_maps = self.feature_extractor(resized_inputs, training=training)

    if self.box_predictor.is_keras_model:
      predictor_results_dict = self.box_predictor(feature_maps, training=training)
    else:
      raise NotImplementedError("Box predictor has to be implemented in Keras")
    predictions_dict = {
      'preprocessed_inputs': resized_inputs,
      'feature_maps':        feature_maps,
      'anchors':             self._anchors.get()
    }
    for prediction_key, prediction_list in iter(predictor_results_dict.items()):
    ...
    if not training:
      predictions_dict['detections'] = self.postprocessor(predictions_dict, true_image_shapes)
  ...
  def anchors(self):
    ...
    if not self._anchors:
      raise RuntimeError('anchors have not been constructed yet! Please use set_anchors method to fully initialize the layer')
    if not isinstance(self._anchors, box_list.BoxList):
      raise RuntimeError('anchors should be a BoxList object, but is not.')
