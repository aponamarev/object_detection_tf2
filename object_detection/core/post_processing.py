...
def batch_iou(boxes1, boxes2):
  ...
  with tf.compat.v1.name_scope('BatchIOU'):
    ...
    intersection_xmin = tf.maximum(x1_min, tf.transpose(a=x2_min, perm=[0, 2, 1]))
    intersection_xmax = tf.minimum(x1_max, tf.transpose(a=x2_max, perm=[0, 2, 1]))
    intersection_ymin = tf.maximum(y1_min, tf.transpose(a=y2_min, perm=[0, 2, 1]))
    intersection_ymax = tf.minimum(y1_max, tf.transpose(a=y2_max, perm=[0, 2, 1]))
    ...
    union_area = area1 + tf.transpose(a=area2,
                                      perm=[0, 2, 1]) - intersection_area + 1e-8
    ...
    iou = tf.compat.v1.where(padding_mask, -tf.ones_like(iou), iou)
...
def _self_suppression(iou, iou_threshold, loop_condition, iou_sum):
  ...
  can_suppress_others = tf.cast(
      tf.reshape(tf.reduce_max(input_tensor=iou, axis=1) <= iou_threshold, [1, -1, 1]), iou.dtype)
  iou_suppressed = tf.reshape(
      tf.cast(
          tf.reduce_max(input_tensor=can_suppress_others * iou, axis=1) <= iou_threshold,
          iou.dtype), [1, -1, 1]) * iou
  iou_sum_new = tf.reduce_sum(input_tensor=iou_suppressed, axis=[1, 2])
  return [
      iou_suppressed, iou_threshold,
      tf.reduce_any(input_tensor=iou_sum - iou_sum_new > iou_threshold), iou_sum_new
  ]
...
def _suppression_loop_body(boxes, iou_threshold, output_size, idx):
  ...
  """
  num_tiles = tf.shape(input=boxes)[1] // _NMS_TILE_SIZE
  ...
  suppressed_iou, _, _, _ = tf.while_loop(
      cond=lambda _iou, _threshold, loop_condition, _iou_sum: loop_condition,
      body=_self_suppression,
      loop_vars=[iou, iou_threshold,
       tf.constant(True),
...
def partitioned_non_max_suppression_padded(boxes,
  ...
  pad = tf.cast(
      tf.math.ceil(tf.cast(num_boxes, tf.float32) / _NMS_TILE_SIZE),
  ...
  num_boxes += pad
  boxes = tf.pad(
      tensor=tf.cast(boxes, tf.float32), paddings=[[0, pad], [0, 0]], constant_values=-1)
  scores = tf.pad(tensor=tf.cast(scores, tf.float32), paddings=[[0, pad]])
  ...
  selected_boxes, _, output_size, _ = tf.while_loop(
      cond=_loop_cond, body=_suppression_loop_body,
      loop_vars=[boxes, iou_threshold,
       tf.zeros([1], tf.int32),
       tf.constant(0)])
  ...
  num_valid_boxes = tf.reduce_sum(input_tensor=output_size)
...
def _clip_window_prune_boxes(sorted_boxes, clip_window, pad_to_max_output_size,
  ...
  sorted_boxes_size = tf.shape(input=sorted_boxes.get())[0]
  non_zero_box_area = tf.cast(box_list_ops.area(sorted_boxes), tf.bool)
  sorted_boxes_scores = tf.compat.v1.where(
  ...
  num_valid_nms_boxes_cumulative = tf.reduce_sum(
      input_tensor=tf.cast(tf.greater_equal(sorted_boxes_scores, 0), tf.int32))
...
def multiclass_non_max_suppression(boxes,
  ...
  with tf.compat.v1.name_scope(scope, 'MultiClassNonMaxSuppression'):
    ...
    for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
      ...
      if pad_to_max_output_size:
        ...
        if (hasattr(tf.image, 'non_max_suppression_with_scores') and
          ...
          num_valid_nms_boxes = tf.shape(input=selected_indices)[0]
      ...
      nms_result.add_field(
          fields.BoxListFields.scores,
          tf.compat.v1.where(valid_nms_boxes_indices,
    ...
    if max_total_size:
      ...
      num_valid_nms_boxes_cumulative = tf.compat.v1.where(
...
def class_agnostic_non_max_suppression(boxes,
  ...
  if q > 1:
    class_ids = tf.expand_dims(
        tf.argmax(input=scores, axis=1, output_type=tf.int32), axis=1)
    boxes = tf.gather(boxes, class_ids, batch_dims=-1)
    if masks is not None:
      masks = tf.gather(masks, class_ids, batch_dims=-1)
    if boundaries is not None:
      boundaries = tf.gather(boundaries, class_ids, batch_dims=-1)
  ...
  with tf.name_scope(scope or 'ClassAgnosticNonMaxSuppression'):
    boxlist_and_class_scores = box_list.BoxList(boxes)
    max_scores = tf.reduce_max(input_tensor=scores, axis=-1)
    classes_with_max_scores = tf.argmax(input=scores, axis=-1)
    ...
    if max_total_size:
      max_total_size = tf.minimum(max_total_size, sorted_boxes.num_boxes())
      sorted_boxes = box_list_ops.gather(sorted_boxes, tf.range(max_total_size))
      num_valid_nms_boxes = tf.compat.v1.where(max_total_size > num_valid_nms_boxes,
...
def batch_multiclass_non_max_suppression(boxes,
  ...
  with tf.name_scope(scope or 'BatchMultiClassNonMaxSuppression'):
    ...
    def _single_image_nms_fn(args):
      ...
      if use_static_shapes:
        total_proposals = tf.shape(input=per_image_scores)
        per_image_scores = tf.compat.v1.where(
