...
def prune_outside_window(boxlist, window, scope=None):
  ...
  with tf.name_scope(scope or 'PruneOutsideWindow'):
    ...
    valid_indices = tf.reshape(
        tf.where(tf.logical_not(tf.reduce_any(input_tensor=coordinate_violations, axis=1))), [-1])
...
def ioa(boxlist1, boxlist2, scope=None):
  ...
  with tf.name_scope(scope or 'IOA'):
    ...
    return tf.math.truediv(intersections, areas)
  ...
  with tf.name_scope(scope or 'BooleanMask'):
    ...
    if use_static_shapes:
      ...
      sampled_indices = tf.cast(
          tf.tensordot(
              tf.cast(tf.range(tf.shape(input=indicator)[0]), dtype=tf.float32),
              one_hot_selector,
              axes=[0, 0]),
          dtype=tf.int32)
...
def pad_or_clip_box_list(boxlist, num_boxes, scope=None):
  ...
  """
  with tf.name_scope(scope or 'PadOrClipBoxList'):
...
def select_random_box(boxlist,
  ...
  """
  with tf.name_scope(scope or 'SelectRandomBox'):
    bboxes = boxlist.get()
...
def sample_boxes_by_jittering(boxlist,
  ...
  with tf.name_scope(scope or 'SampleBoxesByJittering'):
    ...
    rand_miny_gaussian = tf.random.normal([num_boxes_to_sample], stddev=stddev)
    rand_minx_gaussian = tf.random.normal([num_boxes_to_sample], stddev=stddev)
    rand_maxy_gaussian = tf.random.normal([num_boxes_to_sample], stddev=stddev)
    rand_maxx_gaussian = tf.random.normal([num_boxes_to_sample], stddev=stddev)
