...
def drop_label_probabilistically(boxes,
  ...
  with tf.compat.v1.name_scope('DropLabelProbabilistically',
    ...
    indices = tf.compat.v1.where(
        tf.logical_or(
            tf.random.uniform(tf.shape(input=labels), seed=seed) > drop_probability,
...
def remap_labels(labels,
  ...
  for original_label in original_labels:
    ...
    new_labels = tf.add(
        new_labels,
        change)
  new_labels = tf.reshape(new_labels, tf.shape(input=labels))
  return new_labels
...
def random_rgb_to_gray(image,
  ...
  with tf.compat.v1.name_scope('RandomRGBtoGray', values=[image]):
    ...
    image = tf.cond(
        ...
        false_fn=lambda: _augment_only_rgb_channels(image, _image_to_gray))
...
def random_absolute_pad_image(image,
  ...
  Returns:
    ...
    boxes: boxes which is the same rank as input boxes. Boxes are in normalized
           form.
  """
  min_image_size = tf.shape(input=image)[:2]
...
def resize_to_range(image,
  ...
  with tf.name_scope('ResizeToRange'):
    if image.get_shape().is_fully_defined():
      ...
      new_image = tf.cond(
          pred=tf.less(tf.shape(input=image)[0], tf.shape(input=image)[1]),
          true_fn=lambda: _resize_landscape_image(image),
          false_fn=lambda: _resize_portrait_image(image))
      new_size = tf.shape(input=new_image)
...
def resize_image(image,
  ...
  with tf.name_scope('ResizeImage'):
    ...
    result.append(tf.stack([new_height, new_width, image_shape[-1]]))
    return result
...
def random_self_concat_image(
  ...
  concat_vertical = (tf.random.uniform([], seed=seed) <
                     concat_vertical_probability)
  # Note the seed + 1 so we get some semblance of independence even with
  # fixed seeds.
  concat_horizontal = (tf.random.uniform([], seed=seed + 1 if seed else None)
  ...
  image, boxes, labels, label_weights = tf.cond(
      pred=concat_vertical,
      true_fn=lambda: _concat_image(image, boxes, labels, label_weights, axis=0),
      false_fn=lambda: [image, boxes, labels, label_weights])

  outputs = tf.cond(
      pred=concat_horizontal,
      true_fn=lambda: _concat_image(image, boxes, labels, label_weights, axis=1),
      false_fn=lambda: [image, boxes, labels, label_weights])

  if label_confidences is not None:
    label_confidences = tf.cond(pred=concat_vertical,
                                true_fn=lambda: tf.tile(label_confidences, [2]),
                                false_fn=lambda: label_confidences)
    outputs.append(tf.cond(pred=concat_horizontal,
                           true_fn=lambda: tf.tile(label_confidences, [2]),
                           false_fn=lambda: label_confidences))

  if multiclass_scores is not None:
    multiclass_scores = tf.cond(pred=concat_vertical,
                                true_fn=lambda: tf.tile(multiclass_scores, [2, 1]),
                                false_fn=lambda: multiclass_scores)
    outputs.append(tf.cond(pred=concat_horizontal,
                           true_fn=lambda: tf.tile(multiclass_scores, [2, 1]),
                           false_fn=lambda: multiclass_scores))
