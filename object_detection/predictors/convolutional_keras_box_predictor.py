...
class ConvolutionalBoxPredictor(box_predictor.KerasBoxPredictor):
  ...
  def restore_from_tf_checkpoint_fn(self, reader) -> dict:
    ...
    for w in self.weights:
      ...
      name_match = name
      if name.startswith("BoxPredictor/ConvolutionalBoxHead_"):
        name_match = name_match.replace("BoxPredictor/ConvolutionalBoxHead_", "BoxPredictor_")

      if name.startswith("BoxPredictor/ConvolutionalClassHead_"):
        name_match = name_match.replace("BoxPredictor/ConvolutionalClassHead_", "BoxPredictor_")

      if 'kernel' in name_match:
        name_match = name_match.replace('kernel', 'weights')
      if '/bias:0' in name_match:
        name_match = name_match.replace('/bias:0', '/biases:0')
