...
class SSDMobileNetV1KerasFeatureExtractor(
  ...
  def restore_from_tf_checkpoint_fn(self, reader) -> dict:
    ...
    for w in self.weights:
      ...
      if name.startswith('FeatureExtractor/FeatureMaps/'):
        name_match = name.replace("FeatureMaps/", "MobilenetV1/")
        ...
        name_match = 'FeatureExtractor/MobilenetV1/'

        if name.startswith("conv1"):
            name_match = name_match + "Conv2d_0/"
            if "_bn/" in name:
                name_match += "BatchNorm/"
            name_match += name.split("/")[1]
        elif name.startswith("conv_dw_"):
            _l = name.replace("conv_dw_", "").split("/")
            name_match += "Conv2d_" + _l[0] + "_depthwise/"
            if "_bn/" in name:
                name_match = name_match.replace("_bn", "")
                name_match += "BatchNorm/"
            name_match += _l[1]
        elif name.startswith("conv_pw_"):
            _l = name.replace("conv_pw_", "").split("/")
            name_match += "Conv2d_" + _l[0] + "_pointwise/"
            if "_bn/" in name:
                name_match = name_match.replace("_bn", "")
                name_match += "BatchNorm/"
            name_match += _l[1]
